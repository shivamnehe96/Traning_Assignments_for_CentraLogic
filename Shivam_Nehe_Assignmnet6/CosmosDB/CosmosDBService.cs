...
namespace EmployeeManagementSystem.CosmosDB
{
    public class CosmosDBService: ICosmosDBInterface
    {
       ...
       public async Task<List<EmployeeAdditionalDetailsEntity>>GetAllEmployeesWithAdditioanlDetails()
        ...
        public Task<EmployeeAdditionalDetailsEntity> GetEmployeeDetails(string uId)
        {
        ...
        {
            var seriallizedObj = JsonConvert.SerializeObject(employeeBasicDetailsDTO);
            var requestObj = await HttpClientHelper.MakePostRequest(Credentials.BasicDetailsUrl, Credentials.AddBasicDetailsEndPoint, seriallizedObj);
        ...
        {
            var url = $"{Credentials.BasicDetailsUrl}/{Credentials.GetBasicDetailsEndPoint}/{employeeId}";
            var responseObj = await HttpClientHelper.MakeGetRequest(url,Credentials.GetBasicDetailsEndPoint);

            var employeeBasicDetailsEntity = JsonConvert.DeserializeObject<EmployeeBasicDetailsEntity>(responseObj);
            return employeeBasicDetailsEntity;
        }
    }
}
